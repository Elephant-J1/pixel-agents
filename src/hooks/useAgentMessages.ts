...
import { vscode, wsClient } from '../api/wsClient.js'
  ...
  useEffect(() => {
    ...
    const handler = (msg: Record<string, unknown>) => {
      ...
      if (msg.type === 'layoutLoaded') {
        ...
        const agentPayloads = incoming.map((id) => {
          const m = meta[id]
          return { id, palette: m?.palette, hueShift: m?.hueShift, seatId: m?.seatId, folderName: folderNames[id] }
        })
        if (layoutReadyRef.current) {
          // Layout already applied — add agents immediately
          for (const p of agentPayloads) {
            os.addAgent(p.id, p.palette, p.hueShift, p.seatId, true, p.folderName)
          }
          if (os.characters.size > 0) saveAgentSeats(os)
        } else {
          for (const p of agentPayloads) pendingAgents.push(p)
        }
        setAgents((prev) => {
    ...
    const unsubscribe = wsClient.onMessage((data) => handler(data as Record<string, unknown>))
    return () => unsubscribe()
  }, [getOfficeState])
