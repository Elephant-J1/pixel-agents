...
function App() {
  ...
  useEffect(() => {
    return wsClient.onConnectionChange((state) => setConnectionState(state))
  }, [])

  const handleToggleDebugMode = useCallback(() => setIsDebugMode((prev) => !prev), [])

  const handleSelectAgent = useCallback((id: number) => {
    setSidebarAgentId((prev) => (prev === id ? null : id))
  }, [])
  ...
  const handleClick = useCallback((agentId: number) => {
    ...
    setSidebarAgentId((prev) => (prev === focusId ? null : focusId))
  }, [])
  ...
  const activeCount = agents.filter((id) => agentStatuses[id] === 'active' || (agentTools[id]?.some((t) => !t.done) ?? false)).length
  const errorCount = agents.filter((id) => agentStatuses[id] === 'error').length
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      {connectionState !== 'connected' && (
        <ConnectionBanner state={connectionState} gatewayState={gatewayState} />
      )}
      <TopBar
      ...
      <BottomToolbar
        ...
        getLayout={() => officeState.getLayout()}
      ...
      {isDebugMode && (
        ...
        {sidebarAgentId !== null && (
          <AgentSidebar
            ...
            agentTools={agentTools}
            agentStatuses={agentStatuses}
            onClose={() => setSidebarAgentId(null)}
          />
        )}
      </div>
    </div>
