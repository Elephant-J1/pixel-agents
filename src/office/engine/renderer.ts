...
export function renderStatusIndicators(
  ...
  for (const ch of characters) {
    ...
    let alpha = 1.0
    if (ch.isActive && status !== 'error') {
      alpha = 0.5 + 0.5 * Math.sin(time * 4)
    }

    const sittingOffset = ch.state === CharacterState.TYPE ? BUBBLE_SITTING_OFFSET_PX : 0
    const dotX = Math.round(offsetX + (ch.x + 8) * zoom)
    const dotY = Math.round(offsetY + (ch.y + sittingOffset - 22) * zoom)
...
export function renderFrame(
  ...
  // Status indicator dots (optional)
  if (agentStatuses !== undefined) {
    renderStatusIndicators(ctx, characters, agentStatuses, offsetX, offsetY, zoom, time ?? 0)
  }
